...
strip_removal_tool = StructuredTool.from_function(
    func=run_strip_removal,
    name="SDGSAT-1_strip_removal_tool",
    description="专门用于去除SDGSAT-1遥感影像中的条带噪声",
...
VNP46A2_NTL_data_preprocess_tool = StructuredTool.from_function(
    VNP46A2_NTL_data_preprocess,
    name="VNP46A2_NTL_data_preprocess",
    description=(
        """
        This tool preprocess VNP46A2 NTL data from Google Earth Engine based on specified parameters, 
        including region name, scale level('country', 'province', 'city', 'county'), and time range('YYYY-MM to YYYY-MM'). 
...
SDGSAT1_preprocess_tool = StructuredTool.from_function(
    func=calibrate_rgb_from_calib_file,
    name="SDGSAT1_preprocess",
    description=(
        "Perform radiometric calibration on an SDGSAT-1 RGB image using an XML calibration file. "
        "Outputs both a calibrated RGB GeoTIFF and a grayscale luminance image derived via perceptual weighting."
        "Example input:"
